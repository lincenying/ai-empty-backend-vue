<template>
    <div class="global-table">
        <el-table
            ...
            ref="tableRef"
            class="system-table"
            :class="!tableConfig.border ? 'table-top-border' : ''"
            :data="data"
...
import type { DataType, TypeMap } from '~/types/components.types'
import type { GlobalTableProps, UpdatePageType } from '~/types/global.types'
...
const {
    ...
    showSelection = false,
    showPage = true,
    page = { currPage: 1, pageSize: 15, total: 0 },
...
const currPage = ref<number>(page.currPage || 1)
...
function handleCurrentChange(val: number) {
    ...
    emit('updatePage', { key: 'currPage', value: timer ? 1 : val })
}
...
function handleSizeChange(val: number) {
    ...
    currPage.value = 1
    emit('updatePage', [
        { key: 'pageSize', value: val },
        { key: 'currPage', value: 1 },
    ])
}

/** 选择监听器 */
function onSelectionChange(val: DataType<T>[]) {
